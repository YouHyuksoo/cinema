...
describe('screen command contract', () => {
  ...
  it('maps every operator-adjustable screen setting to a deterministic command', () => {
    const cases: [string, string, string][] = [
      ...
      ['machine', '검사 대상 자동차로 변경해', 'car'], ['barsDimension', '막대 형태 평면으로 변경해', '2d'],
      ['barsDepth', '막대 두께 80퍼센트로 변경해', '80'], ['pieDimension', '파이 형태 입체로 변경해', '3d'],
      ['pieDepth', '파이 두께 120퍼센트로 변경해', '120'], ['camera', '카메라 연결 켜줘', 'true'],
