...
const saved: AiConfig = { provider: 'anthropic', model: 'claude-sonnet-5', apiKey: 'sk-ant-secret', temperature: 0.4, maxOutputTokens: 600,
  instructions: '세 문장 이내로 답할 것.', prompt: '', voicePrompt: '', voiceInstructions: '', realtimeModel: 'gpt-realtime-2.1-mini', voiceMode: 'realtime', voiceGender: 'male' };
...
describe('AI settings model', () => {
  ...
  it('validates provider, model, ranges and instruction length, filling defaults for omitted settings', () => {
    ...
    expect(parsed).toEqual({ ok: true, config: { provider: 'anthropic', model: 'claude-sonnet-5', apiKey: 'k', temperature: 0.46, maxOutputTokens: 900, instructions: '짧게', prompt: '', voicePrompt: '', voiceInstructions: '', realtimeModel: DEFAULT_AI_CONFIG.realtimeModel, voiceMode: 'realtime', voiceGender: 'male' } });
  ...
  it('masks the key and reports where it comes from', () => {
    expect(maskAiConfig(saved, true)).toEqual({ provider: 'anthropic', model: 'claude-sonnet-5', temperature: 0.4, maxOutputTokens: 600,
      instructions: '세 문장 이내로 답할 것.', prompt: '', voicePrompt: '', voiceInstructions: '', realtimeModel: 'gpt-realtime-2.1-mini', voiceMode: 'realtime', voiceGender: 'male', hasApiKey: true, keySource: 'config' });
