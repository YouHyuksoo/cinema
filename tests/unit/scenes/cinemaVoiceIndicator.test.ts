...
describe('scene voice connection indicator', () => {
  it('shows the live phase in a globe-sized corner reactor with an explicit stop control', () => {
    ...
    const html = renderToStaticMarkup(createElement(JarvisVoiceIndicator, { audio, phase: 'listening', theme: 'cyan', onStop() {} }));
  ...
  it('is mounted only outside the main preview while voice remains active', () => {
    const source = readFileSync('src/cinema/SignalFilm.tsx','utf8');
    expect(source).toContain('!preview && turbine.voice.active && <JarvisVoiceIndicator');
  });
});
