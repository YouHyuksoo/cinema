...
export function drawJarvisVoiceField(ctx: CanvasRenderingContext2D, input: VoiceFieldInput, effects: CanvasRenderingContext2D = ctx) {
  ...
  if (egg) {
    if (egg.behind) drawReactorShip(effects, egg);
    drawReactorLaser(effects, state, egg); drawReactorImpact(effects, egg);
  }
  drawArcReactor(ctx, state, effects);
  for (const spark of voiceTeslaSparks(state)) drawTeslaDischarge(ctx, spark, state.pitch);
  if (egg && !egg.behind) drawReactorShip(effects, egg);
  ctx.restore();
}
