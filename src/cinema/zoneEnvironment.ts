...
export function zoneEnvironmentState(time: number, data: ZoneEnvironmentData = DEFAULT_ENVIRONMENT_DATA,
  ...
  const manualIndex = selectedId === null ? -1 : source.findIndex((zone, index) => zone.id === selectedId
    && environmentCardVisibility(elapsed, index).selectable);
  const selectedIndex = manualIndex >= 0 ? manualIndex
    : tourTime >= 0 && elapsed < timing.tourEnd && count ? Math.min(count - 1, Math.floor(tourTime / dwell)) : -1;
  const local = selectedIndex >= 0 ? tourTime - selectedIndex * dwell : 0;
  const focus = manualIndex >= 0 ? 1
    : selectedIndex >= 0 ? smooth(0, dwell * .24, local) * (1 - smooth(dwell * .73, dwell, local)) : 0;
