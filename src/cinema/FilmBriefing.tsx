...
export function FilmBriefing({ text, source }: { text: string; source: string }) {
  ...
  const status = source || '브리핑 대기';
  // One header line: the title on the left, the state on the right; the dialogue's own "HATCHERY · source" line is hidden by the panel CSS.
  return <section className={styles.panel} aria-label="텍스트 브리핑">
    <header><span>TEXT / BRIEFING</span><span data-briefing-status role="status">{status}</span></header>
    <JarvisDialogue key={reply} text={reply} source={status} />
  </section>;
}
