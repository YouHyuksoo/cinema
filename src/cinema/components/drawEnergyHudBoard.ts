...
/** Act one: four glass panels and the corner caption. Alpha fades the act out. */
...
function drawBarsPanel(ctx: CanvasRenderingContext2D, fonts: FilmFonts, data: EnergyCoreData, state: EnergyBoardState, alpha: number) {
  ...
  drawHudPanel(ctx, fonts, rect, { label: `${layer.label} / 24H`, line: P.line, panel: P.panel, ink: P.ink, alpha, grid: false });
...
function drawSpeedGauge(ctx: CanvasRenderingContext2D, fonts: FilmFonts, data: EnergyCoreData, state: EnergyBoardState, time: number, alpha: number) {
  ...
  drawHudPanel(ctx, fonts, rect, { label: `${layer.label} / GAUGE`, line: P.line, panel: P.panel, ink: P.ink, alpha, grid: false, labelBottom: true });
  ...
  ctx.font = `10px ${fonts.label}`; ctx.fillStyle = P.dim; ctx.textAlign = 'center';
  ctx.fillText(`x ${reading.capacity.toLocaleString('en-US')} ${reading.unit}`, cx, cy - 24);
  drawEnergyValue(ctx, fonts, String(Math.round(ratio * 100)), cx + 2, cy + 6, 26, P.channels[0], alpha, { align: 'center' });
...
function drawRingGauges(ctx: CanvasRenderingContext2D, fonts: FilmFonts, data: EnergyCoreData, state: EnergyBoardState, time: number, alpha: number) {
  ...
  drawHudPanel(ctx, fonts, rect, { label: 'CHANNELS / RINGS', line: P.line, panel: P.panel, ink: P.ink, alpha, grid: false });
  ...
  ctx.fillStyle = P.ink; ctx.font = `14px ${fonts.label}`; ctx.textAlign = 'center';
  ...
  smalls.forEach(({ index, x, y, r }) => {
    ...
    ctx.fillStyle = P.ink; ctx.font = `10px ${fonts.label}`; ctx.fillText(`${Math.round(ratio * 100)}%`, x, y + 4);
  });
  ctx.restore();
}
...
function drawHexReadouts(ctx: CanvasRenderingContext2D, fonts: FilmFonts, data: EnergyCoreData, state: EnergyBoardState, alpha: number) {
  ...
  drawHudPanel(ctx, fonts, rect, { label: 'READOUT / VALUES', line: P.line, panel: P.panel, ink: P.ink, alpha, grid: false, labelBottom: true });
  ...
  drawEnergyValue(ctx, fonts, digits, big.x, big.y - 16, 30, P.line, alpha, { align: 'center' });
  ctx.fillStyle = P.dim; ctx.font = `9px ${fonts.label}`; ctx.textAlign = 'center'; ctx.fillText(`${active.label} ${reading.unit}`, big.x, big.y + 36);
  ENERGY_LAYERS.map((_, index) => index).filter(index => index !== state.activeIndex).forEach((index, position) => {
    ...
    drawEnergyValue(ctx, fonts, Number.isFinite(value) ? String(Math.round(value)).slice(-3) : '--', x, y - 8, 15, P.line, alpha, { align: 'center' });
    ctx.fillStyle = P.dim; ctx.font = `8px ${fonts.label}`; ctx.fillText(ENERGY_LAYERS[index].label, x, y + 22);
  });
  ctx.restore();
}
