...
export function JarvisChatTools({ camera, input, onInput, mic }: {
  camera: FilmCamera; input: string; onInput(value: string): void;
  mic: { active: boolean; disabled: boolean; label: string; title: string; error?: string; toggle(): void };
}) {
  ...
  return <div className={styles.tools}>
    ...
    <button type="button" aria-label={mic.label} title={mic.title}
      aria-pressed={mic.active} disabled={mic.disabled} onClick={mic.toggle}>
    ...
    {mic.error && <span className={styles.error} role="alert">{mic.error}</span>}
