...
export function resolveJarvisCommand(input: string): JarvisReply | null {
  ...
  if (/보여|열어|이동|틀어|재생/.test(text)) {
    const aliases: [RegExp, FilmId][] = [[/온습도|온도|습도/, 'wave'], [/spc|공정능력|관리도/, 'spc'],
      [/기어/, 'gears'], [/레이싱|F1|포뮬러|차량|투명 설비|분해/i, 'machine'], [/에너지/, 'energy'],
      [/코너/, 'corners'], [/막대/, 'bars'], [/파이/, 'pie'], [/바이저.*평면/, 'visorPan'], [/바이저/, 'visor']];
    const id = aliases.find(([pattern]) => pattern.test(text))?.[1]
      ?? FILM_CHAPTERS.find(c => text.includes(c.title.toLowerCase()))?.id;
