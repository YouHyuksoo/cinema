...
import { isVoiceGender } from '@/cinema/jarvisVoiceGender';
...
export async function PATCH(request: Request) {
  const rejected = rejectExternalRequest(request); if (rejected) return rejected;
  let raw: { voiceMode?: unknown; voiceGender?: unknown; provider?: unknown; model?: unknown };
  ...
  if (raw?.voiceGender !== undefined && !isVoiceGender(raw.voiceGender)) return Response.json({ error: '목소리 성별은 male 또는 female이어야 합니다.' }, { status: 400 });
  ...
  if (raw?.voiceMode === undefined && raw?.voiceGender === undefined && raw?.provider === undefined && raw?.model === undefined) return Response.json({ error: '바꿀 항목이 없습니다.' }, { status: 400 });
  ...
  const parsed = parseAiConfig({ ...base, provider, model, apiKey: '', voiceMode: isAiVoiceMode(raw.voiceMode) ? raw.voiceMode : base.voiceMode,
    voiceGender: isVoiceGender(raw.voiceGender) ? raw.voiceGender : base.voiceGender });
