...
- `aiProviders.ts`에 Mistral Chat Completions 요청과 응답 추출 분기를 추가한다. 요청은 `POST https://api.mistral.ai/v1/chat/completions`, Bearer 인증, JSON 본문을 사용한다.
...
- Mistral HTTP 오류는 기존 `AiProviderError` 경로로 상태 코드와 안전하게 정리된 메시지를 반환한다.
